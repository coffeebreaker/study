...
min_height = min(min(city) for city in cities)
max_height = max(max(city) for city in cities)

dx = [-1, 1, 0, 0]
dy = [0, 0, -1, 1]


def dfs(visited, rain, cities, x, y):
    visited[x][y] = True

    for i in range(4):  # 상하좌우로 탐색
        nx, ny = x + dx[i], y + dy[i]
        if nx < 0 or nx >= n or ny < 0 or ny >= n:  # 영역을 벗어나는 경우는 제외
            continue
        if cities[nx][ny] > rain and not visited[nx][ny]:  # 높이가 rain보다 크고, 방문하지 않은 지역일 경우
            dfs(visited, rain, cities, nx, ny)


# 비가 min_height-1 만큼 내리면 구역은 하나
spots = 1
for rain in range(min_height, max_height):
    visited = [[False]*n for _ in range(n)]  # 이중 리스트 초기화
    count = 0
    for i in range(n):
        for j in range(n):
            if cities[i][j] > rain and not visited[i][j]:  # 높이가 rain보다 크고, 방문하지 않은 지역일 경우
                dfs(visited, rain, cities, i, j)
                count += 1
    spots = max(spots, count)
