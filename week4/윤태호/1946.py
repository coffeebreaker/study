...
def max_pass(applies, n):
    applies.sort(key=lambda x: (x[0], x[1]))

    cur_score = applies[0][1]
    count = 1
    for scores in applies[1:]:
        if scores[1] < cur_score:
            count += 1
            cur_score = min(scores[1], cur_score)
